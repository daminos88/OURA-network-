...
function withSafeMinOut(candidate, config = {}) {
  ...
  return {
    ...candidate,
    amountOutMin: Math.max(0, amountOutMin)
  };
}
...
function attachAttestation(candidate) {
  ...
  const envelopeRes = buildAttestationEnvelope({
    kind: "EXECUTION_PLAN",
    payload
  });

  const proofRes = buildReplayProof({
    envelope: envelopeRes.envelope
  });
...
export async function runOrchestratedExecutionV1({ rpcUrl, riskConfig = {}, reserveConfig = {}, executionConfig = {} }) {
  ...
  const gasRanked = rerankByTrueExecutableProfit(baseCandidates, {
    ...
    gasPriceGwei: executionConfig.gasPriceGwei ?? 0.1,
    ethPriceUsd: executionConfig.ethPriceUsd ?? null
  });

  const mevRanked = rerankByProtectedProfit(gasRanked, {
    gasPriceGwei: executionConfig.gasPriceGwei ?? 0.1,
    priorityFeeGwei: executionConfig.priorityFeeGwei ?? 0.01
  });

  const guarded = applyExecutionGuard(mevRanked, {
  ...
  const attested = relayPlanned.map((c) => attachAttestation(c));
