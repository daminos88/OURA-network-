...
import { buildSignerRequest, buildSignedEnvelope, validateSignerResponse } from "./signer_layer_v1.js";
...
function routeOuraDecision(candidate, config = {}) {
  const decision = candidate?.oura?.decision ?? "REJECT";

  if (decision === "DECAY") {
    const decaySizeFactor = Number(config.decaySizeFactor ?? 0.5);
    return {
      ...candidate,
      size: Number(candidate?.size ?? 0) * decaySizeFactor,
      ouraRoute: "DECAY_REDUCED"
    };
  }

  if (decision === "CONTAMINATED") {
    const confirmed = Boolean(config.requireSecondConfirmation);
    return {
      ...candidate,
      contaminationConfirmed: confirmed,
      ouraRoute: confirmed ? "CONTAMINATED_CONFIRMED" : "CONTAMINATED_BLOCKED"
    };
  }

  return {
    ...candidate,
    ouraRoute: decision === "ACCEPT" ? "ACCEPTED" : "REJECTED"
  };
}

function buildNeutralRetry(candidate) {
  return {
    ...candidate,
    retry: {
      ok: true,
...
function attachSigning(candidate, config = {}) {
  ...
  const signerEnabled = Boolean(config.signerEnabled);
  const signerResponse = signerEnabled
    ? {
        accepted: true,
        signatureCount: Array.isArray(candidate?.relayPlan?.dispatchPlan) ? candidate.relayPlan.dispatchPlan.length : 0,
        signerRef: config.signerRef ?? "external-signer-placeholder"
      }
    : {
        accepted: false,
        signatureCount: 0,
  ...
  const signerValidation = validateSignerResponse(signerResponse);
...
export async function runOrchestratedExecutionV5D({ rpcUrl, riskConfig = {}, reserveConfig = {}, executionConfig = {}, calibrationHistory = [] }) {
  ...
  const routed = withOura.map((c) => routeOuraDecision(c, executionConfig));

  const ouraAllowed = routed.filter((c) => {
    const d = c?.oura?.decision;
    if (d === "ACCEPT") return true;
    if (d === "DECAY") return true;
    if (d === "CONTAMINATED") return c?.contaminationConfirmed === true;
    return false;
  });
  ...
  const nonceConsistent = nonced.every((c) => {
    const reservation = c?.nonceReservation;
    if (reservation == null) return false;
    if (typeof reservation === "boolean") return reservation;
    if (typeof reservation?.consistent === "boolean") return reservation.consistent;
    return false;
  });
  ...
  const retried = (executionConfig.initialErrorType == null)
    ? mempool.map((c) => buildNeutralRetry(c))
    : applyRetryPolicy(
        mempool.map((c) => ({
          ...c,
          errorType: executionConfig.initialErrorType,
          attempt: 1
        })),
        {
  ...
  const retryTerminal = signed.some((c) => Boolean(c?.retry?.terminal));
